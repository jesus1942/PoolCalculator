...
export const ProjectCosts: React.FC<Props> = ({project,canEdit=false,onReload,onDirtyChange}) => {
  ...
  return <div className="pcost">
    ...
    {canEdit && <section className="pcost-panel"><h3>3. Catálogo de presets por categoría</h3><p>Definí tus tarifas de instalación y, por separado, los materiales para presupuestos completos. Los presets personalizados se guardan en esta obra al presionar Guardar costos.</p><fieldset disabled={saving}>
    <CostPresetCatalog saved={draft.presets} onAdd={add} onSave={line=>change({...draft,presets:[...draft.presets.filter(item=>item.id!==line.id),line]})} onRemove={id=>change({...draft,presets:draft.presets.filter(item=>item.id!==id)})}/>
